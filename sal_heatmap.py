...
with open(result_file, 'rb') as read_file:
...
for model_name, sal in result.items():
    model_name = model_name.rsplit('.', 1)[0]
    train_method, lambda_1, lambda_2 = model_name.split('_')
    heatmap_array[value_index[lambda_1]][value_index[lambda_2]] = round(sal, 2)
...
plt.savefig('saliency/heatmap_exp4.png', format='png')
plt.show()
